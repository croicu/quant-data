...
def build_findings_markdown(
    ...
    lines.append("")
    lines.append("**Units mismatch, and a marginal-discovery framing (2026-08-26 second review pass, R2)**: precision above is")
    lines.append("counted per (bar, field) -- e.g. k=3's 104 field flags can exceed its 42 whistleblower-caught *bars* because one bar")
    lines.append("can contribute flags on multiple fields. Read the precision percentages only within a fixed k, never compared")
    lines.append("across the precision/recall columns as if they shared a denominator. On the corrected numbers, k=1 nets 93 genuine")
    lines.append("(bar, field) hits against k=3's 86 -- **only 7 more genuine hits for roughly 19x the review volume (2,016 vs 104")
    lines.append("field flags)** -- a cleaner argument for k=3.0 than either percentage alone.")
    lines.append("")
    lines.append("**Is the precision proxy just measuring yfinance-IBKR affinity, not correctness? (R1/B4) -- checked, not")
    lines.append("disqualifying at k=3.** E6b found yfinance tracks ibkr closely on disagreement bars (`im_my=-0.927`), raising the")
    lines.append('concern that a "decisive" flag might just mean "yfinance agreed with ibkr" rather than genuinely picking the')
    lines.append("correct side. Checked directly: among k=3's 86 decisive field flags, yfinance sides with ibkr on 58.1% (50) and")
    lines.append("with massive on 41.9% (36) -- a real lean toward ibkr, but far from the near-automatic 90%+ split B4's concern")
    lines.append("would predict if the proxy were purely measuring affinity rather than correctness. **The split does widen sharply")
    lines.append("at higher k** (59% at k=1-2, 58% at k=3, rising to 84% at k=8 and 93% at k=10) -- so the affinity concern is real")
    lines.append("and grows at the tail, but at the recommended k=3.0 specifically it is not the dominant driver of the precision")
    lines.append("number. **k=3.0 does not need to be held conditional on further B4 work; the high-k end of the grid should be")
    lines.append("read with more caution than the k=3 recommendation itself.**")
    lines.append("")
    lines.append('**Null baseline for precision (R4)**: the unconditional "decisive" rate over all 29,680 (bar, field) instances in')
    lines.append("the overlap window (not just MAD-flagged ones) is **0.313%** -- the correct like-for-like chance comparator (same")
    lines.append("units as the precision column, unlike the whistleblower's bar-level 8.3% flag rate, which is a different")
    lines.append("denominator). Every k in the grid clears this by a wide margin: even k=1's corrected 4.6% is ~15x the null rate,")
    lines.append("and k=3's 82.7% is ~264x it. **No operating point tested is anywhere near indistinguishable from chance.**")
    ...
    lines.append("")
    lines.append("**Which month carries the 18.1% figure, and is it estimator noise or a real event? (2026-08-26 second review pass,")
    lines.append("R3) -- it is January, not March**, and it is not noise: checked directly, January's own-threshold rate (18.07%) is")
    lines.append("driven almost entirely by `high`/`low` (1,791 and 1,724 of 18,388 bars, ~9.4-9.7% each) while `open`/`close` barely")
    lines.append("flag at all (63 and 56 bars, ~0.3%). It is also sustained through the whole month, not front-loaded on the dataset's")
    lines.append("first few days (day-by-day range 12.6%-23.1%, first-5-trading-days average 18.8% vs. the remaining 15 days' 17.8%")
    lines.append("-- no onboarding-artifact signature). **This looks like a real, sustained monthly regime difference in `high`/`low`")
    lines.append("agreement, not conditional-MAD sampling noise** (n=18,388 is not a small sample). March's own outlier (still")
    lines.append("unexplained, open question #5) is a separate, smaller anomaly (1.41% own-threshold) and is NOT closed by this --")
    lines.append("that question remains open.")
    lines.append("")
    lines.append("**Consequence for the recalibration-cadence follow-up (section 8) -- resolved (R3a-R3f)**: pool the")
    lines.append("conditional-MAD *scale* (this section's basis) and keep it fixed; keep `k` rolling against the refreshing")
    lines.append("yfinance window (unaffected -- a different quantity, see section 8); add a separate drift monitor that")
    lines.append("recomputes each month's own conditional MAD and own-threshold rate and alerts on deviation without feeding back")
    lines.append("into the band. Rolling the scale itself would make drift structurally undetectable (every month would flag near")
    lines.append("its own design rate by construction) and is wrong in an unknowable direction (a degrading provider silently")
    lines.append("widens its own band); pooling is wrong in a knowable, observable direction instead.")
    lines.append("")
    lines.append("**Was January/August's pattern actually unmodeled volatility rather than a real regime difference? Checked and")
    lines.append("refuted.** The hypothesis: `high`/`low`'s outsized flag rates (vs. `open`/`close`'s near-zero ones) could reflect")
    lines.append("disagreement scaling with intrabar range rather than a genuine data-quality difference, which would have dissolved")
    lines.append("this tension instead of requiring a decision. Tested directly: per-bar correlation between `|d_high|`/`|d_low|` and")
    lines.append("intrabar range is ~0 (r=0.004-0.013, even restricted to disagreement-only bars); month-level, own-threshold flag")
    lines.append("rate vs. mean intrabar range is r=-0.32 (wrong sign) -- **August has the *lowest* mean intrabar range of all 8")
    lines.append("months despite being the second-most-elevated month for disagreement**, and March has the *highest* range but one")
    lines.append("of the *lowest* disagreement rates. January and August are not the high-volatility months this hypothesis needs")
    lines.append("them to be. **Confirms January/August are genuine data regimes, not a price-level-normalization artifact** -- no")
    lines.append("renormalization work is warranted on this evidence.")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("")
    lines.append("**Revised 2026-08-26 (R3a-R3f) from the original proposal below** -- the original cadence rolled the")
    lines.append("conditional-MAD *scale* monthly, which section 4's investigation shows is the wrong quantity to roll (it makes")
    lines.append("drift structurally undetectable, since every month would then flag near its own design rate by construction).")
    lines.append("Three separate things could roll, and they resolve in opposite directions:")
    lines.append("")
    lines.append("- **Conditional-MAD scale: POOLED, fixed, not rolled.** The dispersion estimate underlying the band should")
    lines.append("  come from the full pooled range (E4's basis), not a trailing month. Standard control-chart reasoning:")
    lines.append("  control limits are fixed from a reference period specifically so drift can be detected against them;")
    lines.append("  recomputing them from the data being monitored makes the chart unable to ever fire. Pooling is also wrong")
    lines.append("  in a *knowable* direction (too loose in quiet months, too tight in volatile ones, and which is observable)")
    lines.append("  -- rolling would be wrong in an *unknowable* direction instead (a degrading provider silently widens its own")
    lines.append("  band with an unchanged flag rate).")
    lines.append("- **`k` (the operating point): rolling, as originally proposed.** Re-picking `k` against fresh yfinance labels")
    lines.append("  doesn't touch the dispersion estimate -- it re-chooses where to sit on a curve E6 measures. yfinance's")
    lines.append('  ~30-day rolling window is the only mechanism for this (per the task\'s "Role of yfinance after this task"')
    lines.append("  section). Monthly, re-run E6's precision/recall proxy against that month's fresh overlap, and only change")
    lines.append("  the production `k` if the recommendation shifts by more than one grid step (E4_K_GRID granularity). Caveat:")
    lines.append("  `k` rolled on recent data gets applied to a historical period with no labels, inheriting E5's flatness")
    lines.append("  result as an assumption rather than validating it directly there.")
    lines.append("- **Drift monitor (new, not in the original proposal): recomputes each month's own conditional MAD and")
    lines.append("  own-threshold flag rate and *alerts* on deviation without changing the band.** This is what recovers the")
    lines.append("  drift-detection capability that pooling the scale would otherwise cost -- an alert triggers a human")
    e5_min = _fmt_pct(e5["verdicts"][tickers[0]]["min_flag_rate_pct"], 1)
    e5_max = _fmt_pct(e5["verdicts"][tickers[0]]["max_flag_rate_pct"], 1)
    lines.append("  recalibration decision (consistent with Pass 2 already being manual/deliberate), not an automatic band")
    lines.append(f"  change. Reference range from E5: {e5_min}-{e5_max} across 8 months.")
    lines.append("")
    lines.append("**Why not renormalize instead of deciding this?** Checked and refuted -- see section 4's own resolution: January's")
    lines.append("and August's elevated flag rates are NOT explained by intrabar-range/volatility scaling (per-bar correlation ~0,")
    lines.append("month-level correlation wrong-signed, August is the *lowest*-volatility month despite being the second-most-elevated")
    lines.append("for disagreement). They are genuine data regimes, which is why pooling the scale (not renormalizing it) is")
    lines.append("the right fix.")
    lines.append("")
    lines.append("**Separate, unresolved follow-up**: whether `high`/`low` need their own field group and `k`, distinct from")
    lines.append("`open`/`close` (E6/E7 already do this for volume) -- January's own-threshold split (~9.5% vs ~0.3%) suggests they")
    lines.append("might. Independent of the scale-vs-k question above either way.")
    lines.append("")
    lines.append("---")
    lines.append("")
