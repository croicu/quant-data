...
def build_findings_markdown(
    ...
    lines.append("")
    lines.append("**Consequence for the recalibration-cadence follow-up (section 8)**: because January's elevated tail is real and")
    lines.append("sustained rather than noise, a purely rolling monthly recalibration is genuinely exposed to landing on a month")
    lines.append("like January and inheriting its (real, not spurious) `high`/`low` looseness -- the same mechanism already observed")
    lines.append('with August. This is a real tension with the "monthly recalibration" cadence proposed in section 8, not fully')
    lines.append("resolved here -- see that section's own note.")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("")
    lines.append("**Open tension, not resolved here (2026-08-26 second review pass, R3)**: section 4's finding that January's")
    lines.append("elevated own-threshold rate is a real, sustained monthly regime difference (not conditional-MAD sampling noise)")
    lines.append("means a purely rolling single-month calibration is genuinely exposed to inheriting a month's real idiosyncrasy")
    lines.append("(as already observed with both January and August) rather than tracking genuine drift. That pulls toward")
    lines.append("calibrating on the pooled full range instead (E4's basis) for stability -- but pooling also can't detect real")
    lines.append("drift if the underlying disagreement characteristics genuinely shift over time, which is the entire reason E5")
    lines.append("exists. **This is a real design choice between the two E0-E8 already computed, not a bug to fix -- flagged for")
    lines.append("the repo owner rather than decided unilaterally before this cadence is actually built.**")
    lines.append("")
    lines.append("---")
    lines.append("")
