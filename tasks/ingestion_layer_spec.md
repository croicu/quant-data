...
Status: design converged on grain and structure; variable availability confirmed, the IBKR serialization question resolved, the `provider_source_archive` schema change done as a coalesced migration, and (2026-08-19) `ingest` itself now actually fetches/archives three IBKR methods (`TRADES` + `BID_ASK` + `MIDPOINT`) by default, live-verified end to end — see §2/§3/§4/§6/§7/§8. This is the ingestion (landing zone) layer only — parsing/extraction into `fact_market_data_1min` and downstream reconciliation are separate, later concerns. Supersedes `tasks/quote_bar_ingest.md` as the working design doc for croicu/quant-data#60 — that file's problem statement still stands but its open questions are answered here.
